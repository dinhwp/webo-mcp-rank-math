...
function webo_rank_math_config_mutate( $input ) {
	...
	return match ( $action ) {
		...
		default               => new WP_Error( 'webo_mcp_invalid_action', sprintf( 'Unknown config-mutate action: %s. Semantic actions (apply-brand-profile, migrate-brand, brand-cleanup, etc.) are available via webo-rank-math/semantic-action.', $action ) ),
	};
}
